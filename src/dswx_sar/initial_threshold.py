...
def run(cfg):
    ...
    for iter_ind in range(threshold_iteration):
        ...
        #                         np.percentile(sample_test, 95) +  int_sub_std * 2,
        #                         int_sub_mean + int_sub_std * 2])
        #                 else:
        #                     thres_max_temp = threshold_otsu(sample_test)
        #                     int_sub_mean = np.nanmean(sample_test[sample_test > thres_max_temp])
        #                     int_sub_std = np.nanstd(sample_test[sample_test > thres_max_temp])
        #                     thres_max[band_ind] = np.nanmax([
        #                         np.percentile(sample_test, 95) +  int_sub_std * 2,
        #                         int_sub_mean + int_sub_std * 2])
        #             else:
        ...
        #                 thres_max[band_ind] = np.nanmax([
        #                     np.percentile(int_water_array_db, 95) +  int_sub_std * 2,
        #                     int_sub_mean + int_sub_std * 2])
        #         else:
        #             int_sub_mean = np.nan
        #             int_sub_std = np.nan
