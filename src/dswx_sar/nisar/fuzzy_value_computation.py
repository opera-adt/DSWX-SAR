...
def zmf_recenter(values, peak, valley, alpha=1.0, min_width=1e-3):
    ...
    the point peak + alpha*(valley - peak).

    alpha=0.0 -> 0.5 at peak
    alpha=0.5 -> 0.5 midway
    alpha=1.0 -> 0.5 at valley
    """
    # target point where membership should be 0.5
    mid = peak + alpha * (valley - peak)
    # since 0.5 happens at (minv + maxv)/2 and minv=peak
    adj_maxv = 2.0 * mid - peak
    # keep ordering and avoid division-by-zero in zmf
    adj_maxv = np.maximum(adj_maxv, peak + min_width)
    return zmf(values, peak, adj_maxv)
...
def compute_fuzzy_value(intensity,
    ...
    for int_id, pol in enumerate(pol_list):
        ...
        # Change : valley to map to ~0.5 with the current zmf implementation
        temp = zmf_recenter(
            intensity_band,
...
def run(cfg):
    ...
    option_dict = {'hand_threshold': processing_cfg.hand.mask_value,
                   ...
                   'intensity_mid_alpha': fuzzy_cfg.intensity.intensity_mid_alpha
                   }
