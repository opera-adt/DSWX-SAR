...
class histogramMetrics:
    def __init__(self,
        ...
        bins_hist = np.linspace(hist_min,hist_max,hist_num+1)
        ...
        self.threshold_global_otsu = threshold_otsu(int_db, nbins=150)

        if self.threshold_global_otsu < - 18:
            multithreshold_global_otsu = threshold_multiotsu(int_db, nbins=150)
            if any(element for element in multithreshold_global_otsu if element > -18):
                self.threshold_global_otsu_ind = np.where((multithreshold_global_otsu > -18) &
                                         (multithreshold_global_otsu < 0))
                self.threshold_global_otsu = multithreshold_global_otsu[self.threshold_global_otsu_ind[0][0]]
    ...
    def compute_Ashman(self):
        ...
        """
        numerator = np.sqrt(2) * np.abs(self.first_mode[0] - self.second_mode[0])
        denominator = np.sqrt(self.first_mode[1] ** 2 + self.second_mode[1] ** 2)
    ...
    def compute_surface_ratio(self):
        """Compute the Surface Ratio coefficient for bimodality estimation.

        This method calculates the Surface Ratio coefficient, 
        which is a measure of bimodality, based on
        the areas under the first and second modes of the simulated data histograms.
        ...
        SRcoeff = np.nanmin([area_first, area_second]) / np.max([area_first, area_second])
        return SRcoeff

    def compute_bc_coefficient(self):
        """Compute the BC coefficient for bimodality estimation.

        This method calculates the BC coefficient, which is a measure of bimodality,
      ...
      """
        ...
        skewness_sq = stats.skew(self.int_db, nan_policy='omit', bias=False) ** 2
        kurtosis = stats.kurtosis(self.int_db, nan_policy='omit', bias=False)
        adjustment = 3 * ((sample_size - 1) ** 2) / (sample_size - 2) / (sample_size - 3)
    ...
    def compute_bimodality(self):
        ...
        This method estimates the bimodality coefficient, which measures the degree of bimodality in the
        input data. It uses histogram analysis to identify the bin corresponding to the valley between the
        two modes of the data. Then, it calculates the bimodality coefficient based on the mean values and
    ...
    def compute_metric(self, 
        ...
        This method computes the bimodality metric based on different criteria and thresholds. It takes
        several flags to control the computation of specific metrics. If optimization is enabled, it uses
        pre-computed metrics stored in the object's attributes. Otherwise, it calculates the bimodality
        metric using the `estimate_bimodality` function.

        Parameters:
            ashman_flag (bool): Flag to indicate whether to consider the Ashman coefficient in the metric.
            bhc_flag (bool): Flag to indicate whether to consider the Bhattacharyya coefficient in the metric.
            bm_flag (bool): Flag to indicate whether to consider the bimodality coefficient in the metric.
            surface_ratio_flag (bool): Flag to indicate whether to consider the surface ratio in the metric.
            bc_flag (bool): Flag to indicate whether to consider the BC coefficient in the metric.
            thresholds (list): A list containing threshold values for different metrics. # ashman, bhc, surface_ratio, bm

        Returns:
            bool: A boolean value indicating whether the data satisfies the bimodality condition based on the
                selected metrics and thresholds.
        """
        ashman, bhc, surface_ratio, bm_coeff, bc_coeff = (None, None, None, None, None)
        ...
        if self.optimization:
            ...
            ashman_bool = ashman_flag and (ashman is None or ashman > thresholds[0])
            bhc_bool = bhc_flag and (bhc is None or bhc > thresholds[1])
            surface_ratio_bool = surface_ratio_flag and (surface_ratio is None or surface_ratio > thresholds[2])
            bc_coeff_bool = bc_flag and (bc_coeff is None or bc_coeff > 5/9)
            bimodal_metrics = (int(ashman_bool) + int(bm_coeff_bool) + int(bc_coeff_bool))
    ...
    def get_metric(self):
        ...
        if self.optimization:
            ...
            Btmax, _ = estimate_bimodality(self.int_db)
            bc_coeff = self.compute_bc_coefficient()
            return [0, 0, 0, Btmax, bc_coeff]

def estimate_bimodality(array, min_im=-30, max_im=5, numstep=100):
    ...
    counts_smooth = scipy.signal.convolve(counts,[0.2261, 0.5478, 0.2261], 'same')
    ...
    for bin_idx, value in enumerate(bincenter):
        ...
        if left_sum > 0 and right_sum > 0:
            ...
            stdp1 = np.sqrt(np.nansum(((counts_smooth[cand1] * bincenter[cand1]) - meanp1 )**2)) / left_sum
            stdp2 = np.sqrt(np.nansum(((counts_smooth[cand2] * bincenter[cand2]) - meanp2 )**2)) / right_sum

            probp1 = np.nansum(counts_smooth[cand1]) / countsum
            probp2 = np.nansum(counts_smooth[cand2]) / countsum

            sigma_b[bin_idx] = probp1 * probp2 * ((meanp1-meanp2)**2) / std_int
            ads[bin_idx] = np.sqrt(2) * (np.abs(meanp1 - meanp2)) / np.sqrt((stdp1**2 + stdp2**2))
...
def process_dark_land_component(args):
    ...
    This function takes a set of input arguments and processes a dark land component
    using various raster datasets such as landcover, intensity bands, binary water mask,
    land areas, and labeled water elements. It computes the bimodality metric for the
    component based on certain conditions and returns the results.

    Parameters:
        args (tuple): A tuple containing the following elements:
            - i (int): The index of the dark land component.
            - sizes (int): The size of the dark land component in pixels.
            - bounds (tuple): The bounding box of the component (row, col, width, height).
            - ref_land_str (str): File path to the raster dataset representing land areas.
            ...
            - water_label_str (str): File path to the labeled water elements raster dataset.
            - debug_mode (bool): Flag indicating whether to enable debug mode.

    Returns:
        tuple: A tuple containing the results for the dark land component:
            - i (int): The index of the dark land component.
            - bimodality_array_i (bool): True if bimodality metric is computed successfully,
                False otherwise.
            - ref_land_portion (float): The portion of the land within binary water elements.
            - metric_output_i (numpy.ndarray): An array of size 5 containing metric output
                values if debug_mode is True, otherwise, it contains zeros.

    """
    i, sizes, bounds, ref_land_str, landcover_str, \
        pol_ind, bands_str, water_label_str, debug_mode = args
    ...
    if sizes >= minimum_pixel and not np.isnan(ref_land_portion):
        ...
        if ref_land_portion > 0.8:
            ...
            mask_buffer = ndimage.binary_dilation(watermask==1,
                                                  iterations=margin,
                                                  mask=out_boundary)
            bands = np.squeeze(bands[pol_ind,:,:])

            intensity_center = np.nanmedian(bands[watermask==1])
            intensity_adjacent_area = bands[(watermask==0) & (mask_buffer==1)]
            intensity_adjacent_low = np.nanpercentile(intensity_adjacent_area, 15)
            
            # if polygons is brighter than adjancet pixels
            ...
            if intensity_center > intensity_adjacent_low:
                ...
                metric_obj = histogramMetrics(intensity_array)
...
def process_bright_water_component(args):
    ...
    Parameters:
        args (tuple): A tuple containing the following elements:
            ...
            - bounds (tuple): The bounding box of the component (row, col, width, height).
            - output_water_str (str): File path to the labeled water elements raster dataset.
            - landcover_str (str): File path to the landcover raster dataset.
            - bands_str (str): File path to the intensity bands raster dataset.
            - ref_land_str (str): File path to the raster dataset representing land areas.
            - pol_ind (int): Index of the polarization band to process.
            - threshold (tuple): A tuple containing two threshold values for Bt and Ad metrics.

    Returns:
        tuple: A tuple containing the following results for the bright water component:
    ...
    image_paths = [landcover_str, bands_str, output_water_str, ref_land_str]
    image_set = []
    ...
    if landcover_portion > 0.99:
        ...
        if margin < 5: 
            margin = 5
        mask_buffer = ndimage.binary_dilation(mask_water,
                        iterations=margin,
                        mask=adjacent_areas)

        intensity_array = bands[mask_buffer]
        Btmax, ADmax = estimate_bimodality(10 * np.log10(intensity_array))

        return Btmax, ADmax, i
    else:
        return Bt_fail, Ad_fail, i
...
def remove_false_water_bimodality_parallel(bands,
    ...
    '''estimate bimodality and normalized sepera
    ...
    for pol_ind, pol in enumerate(pols):
        ...
        if pol in ['VV', 'VH', 'HH', 'HV']:
            ...
            args_list = [(ind_list[i], size_list[i], coord_list[i],
                          ...
                          input_dict['intensity'], water_label_str, debug_mode)
            ...
            if debug_mode:
                ...
                dswx_sar_util.save_raster_gdal(raster=ref_land_portion_image,
                ...
                for metric_ind, metric_name in enumerate(metric_detail_name):
                    ...
                    dswx_sar_util.save_raster_gdal(raster=metric_image0,
                                    output_file=os.path.join(outputdir,
                                    metric_name),
                                    geotransform=meta_info['geotransform'],
                                    projection=meta_info['projection'],
        ...
        if pol in ['VV', 'VH', 'HH', 'HV']:
            ...
            results = Parallel(n_jobs=10)(delayed(
                                        process_bright_water_component)(args)
                                            for args in args_list)
            for res in results:
                Btmax, ADmax, result_ind = res
                bimodality_output[result_ind] = Btmax
                ad_output[result_ind] = ADmax
...
def run(cfg):
    ...
    ref_land_str = os.path.join(outputdir,
                                'landcover_not_water_{}.tif'.format(pol_str))
    dswx_sar_util.save_raster_gdal(raster=landcover_not_water,
                    output_file=ref_land_str,
                    geotransform=im_meta['geotransform'],
                    projection=im_meta['projection'],
                    scratch_dir=outputdir)

    # # If the landcover is non-water, compute the bimnodality one more time and remove the water body if test fails.
    ...
    no_data_raster = np.isnan(np.squeeze(np.nanmean(band_set, axis=0)))
    no_data_raster = no_data_raster | (water_mask_image == 255)
    water_tif_str = os.path.join(outputdir, f"bimodality_output_binary_{pol_str}.tif")
    ...
    if processing_cfg.debug_mode:
        ...
        output_str = os.path.join(outputdir,
            'bimodality_output_comparison_{}.tif'.format(pol_str))
