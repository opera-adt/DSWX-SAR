...
from dswx_sar.common import _dswx_geogrid, _generate_log
from dswx_sar.common._mosaic import mosaic_single_output_file
from dswx_sar.sentinel1.dswx_runconfig import _get_parser, RunConfig
...
def run(cfg):
    ...
    if mosaic_flag:
        ...
        for ind, input_dir in enumerate(input_list):
            ...
            if first_rtc_path:
                rtc_meta = _dswx_sar_util.get_meta_from_tif(first_rtc_path)
                epsg_list.append(rtc_meta['epsg'])
            else:
                err_msg = 'RTC files not found.'
                raise FileExistsError(err_msg)

        epsg_output = _dswx_sar_util.majority_element(epsg_list)
        geogrid_in = _dswx_geogrid.DSWXGeogrid()
        ...
        for ind, input_dir in enumerate(input_list):
            ...
            if len(layover_path) > 0:
                ...
                if epsg_output != epsg_list[ind]:
                    ...
                    _dswx_sar_util.change_epsg_tif(
                ...
                for input_ind, input_dir in enumerate(input_list):
                    ...
                    if rtc_path_inputs:
                        ...
                        if epsg_output != epsg_list[input_ind]:
                            ...
                            _dswx_sar_util.change_epsg_tif(rtc_path_input,
                                                          rtc_path_temp,
                                                          epsg_output)
