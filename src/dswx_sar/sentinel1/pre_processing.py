...
import logging
import mimetypes
from dswx_sar.common import _dswx_sar_util, _filter_SAR, _generate_log
import numpy as np
import os
import pathlib
import shutil
...
from dswx_sar.common import (_pre_processing)
...
def run(cfg):
    ...
    for anc_type, anc_filename in relocated_ancillary_filename_set.items():
        ...
        validation_result = _pre_processing.validate_gtiff(
            os.path.join(scratch_dir, anc_filename),
            [no_data, np.inf])
        ...
        if validation_result not in ['okay']:
            if (anc_type in ['dem', 'hand']) and \
               (validation_result in ['nan_value', 'invalid_only']):
                ...
                # are invalid.
                _pre_processing.replace_reference_water_nodata_from_ancillary(
    ...
    for block_ind, block_param in enumerate(block_params):
        ...
        for polind, pol in enumerate(pol_list):
            ...
            if pol in ['ratio', 'span']:
                ...
                if pol in ['ratio']:
                    ratio = _pre_processing.pol_ratio(
